...
import shaders.CameraGlitch.CameraGlitchEffect;
...
class PlayState extends MusicBeatState
{
	...
	var itemsShader:CameraGlitchEffect;
	...
	override public function create()
	{
		...
		itemsShader = new CameraGlitchEffect();
		...
		timeBarBG = new AttachedSprite('timeBar1');
		...
		timetex = new FlxText(100, 14, FlxG.width - 400, "", 5);
		timetex.text = songName;

		timetex.setFormat(Paths.font("vcr.ttf"), 15, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timetex.scrollFactor.set();
		timetex.cameras = [camHUD];
		add(timetex);
		...
		doof.cameras = [camHUD];
		if (SONG.song == 'confidence')
		{
			dad.shader = itemsShader.shader;
			trace('LOADING SONG');
		}
	...
	override public function update(elapsed:Float)
	{
		...
		itemsShader.update(elapsed);
		lerpScore = Math.floor(FlxMath.lerp(lerpScore, songScore, CoolUtil.boundTo(elapsed * 15, 0, 1)));
		if(Math.abs(songScore - lerpScore) < 5) lerpScore = songScore;
		switch (curStage)
		{
			case 'tank':
				moveTank(elapsed);
